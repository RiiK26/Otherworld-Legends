...
- **Offset Extraction**: NEVER use `IL2CPP::Class::Utils::GetFieldOffset` in C++ for field offsets, as it breaks when games are obfuscated. Instead, rely on `src/Features/Tools/SignatureDumper.cpp` which dynamically extracts offsets via Capstone machine-code analysis at runtime. Use the generated `Offsets.hpp` variables (e.g. `Offsets::Character_minMoveSpeed`), and make sure this offsets are centralized inside the codebase, so there will be no conflict between offset variables across features. Hardcoded C# internal structures (e.g., `System.String`, `Dictionary`) should be strictly placed under `Offsets::CLR`.
